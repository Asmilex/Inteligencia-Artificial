...
class Entidad {
	...
	public:
	  Entidad(Tipo paramTipo, SubTipo paramSubTipo, Orientacion paramOrient, unsigned int paramX, unsigned int paramY, Objeto3D * mod, Comportamiento * comp, int objX = 1, int objY = 1, int v = 500)
	      : tipo(paramTipo), subtipo(paramSubTipo), orient(paramOrient), f(paramX), c(paramY), modelo(mod), comportamiento(comp), destX(objX), destY(objY), vida(v) {}
	  ...
	  void setPosicion(unsigned int paramX, unsigned int paramY) {f = paramX; c = paramY;}
	  ...
	  unsigned int getX() {return f;}
	  unsigned int getY() {return c;}
    unsigned int getObjX(){return destX;}
    unsigned int getObjY(){return destY;}
    ...
    int fixBateria_sig_accion(unsigned char celda);
